...
async function deleteSpitiQuery(id) {
...
window.deleteSpitiQuery = deleteSpitiQuery;

window.deleteContact = deleteContact;

async function loadSpitiQueries() {

  const snapshot =
    await getDocs(collection(db, "spitiqueries"));
  ...
  renderSpitiQueries(queries);
}

function renderSpitiQueries(queries) {

  const tbody =
    document.getElementById("spitiBody");
  ...
  tbody.innerHTML = queries.map(q => `
    <tr>
      ...
      <td>${q.message || "-"}</td>
      <td>
        <button onclick="deleteSpitiQuery('${q.id}')">
          Delete
        </button>
      </td>
    </tr>
